// Builds dist-content/examples.json — the bundled four-example challenge book.
//
// Run with: node scripts/buildExamplesBook.js

import { writeFileSync, mkdirSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { normaliseGuid } from '../src/utils/guid.js';
...
A rectangular forest of trees occupies the centre of the world. Kara starts outside, facing up, with the forest to her right. Her task: complete one full circuit of the forest, always keeping a tree to her right.
...
This exact same two-state FSM — unchanged — navigates **inside a maze** as well as around the outside of a forest. Both are instances of the right-hand rule. For this challenge, pause the FSM after one full circuit when Kara returns to the start.`;
...
1. Drag a \`count with\` loop and set it to run **24** times — enough for one full circuit of this forest.
2. Inside, use an \`if / else if / else\` block:
   - \`if not tree-right?\` → \`turn-right\`, then move if the path is clear.
   - \`else if tree-front?\` → \`turn-left\`.
   - \`else\` → \`move\`.

## Why a Nested Check?

In Blocks mode a branch can contain another \`if\`, so after turning right you can immediately check whether the new forward direction is clear. The FSM version of this example needed a whole second state (q2) just to remember "I just turned right, now please check." Sequential code skips that bookkeeping for free.

## Surprising Fact

The same right-hand-rule idea navigates **inside a maze** as well as around the outside of a forest. This challenge uses a fixed loop count only so Kara stops after one marked circuit.`;
...
In Python a branch can contain another \`if\`, so after turning right you can immediately check whether the new forward direction is clear. The FSM version of this example needed a whole second state (q2) just to remember "I just turned right, now please check." Sequential code skips that bookkeeping for free.
...
// Ex3 Forest Circler: one full orbit around the forest. The reference
// Blocks/Python solutions run the right-hand rule for 24 iterations;
// FSM users can pause after one lap.
const ex3TargetWorld = makeWorld(
  15, 10,
  4, 4, 'up',
  ex3Trees,
  [],
...
} from '../src/exampleSolutions.js';

const exampleLimits = {
  ex1: { blocks: 14, python: 29, fsmStates: 15, fsmTransitions: 14 },
  ex2: { blocks: 32, python: 65, fsmStates: 33, fsmTransitions: 32 },
  ex3: { blocks: 28, python: 57, fsmStates: 29, fsmTransitions: 28 },
  ex4: { blocks: 7,  python: 15, fsmStates: 8,  fsmTransitions: 7  },
};

const ex1IntermediateCheckpoints = [
  makeWorld(15, 10, 3, 3, 'up',   [[4,2],[4,3],[4,4],[3,1],[1,2]], [], [[2,7]]),
  makeWorld(15, 10, 2, 3, 'down', [[4,2],[4,3],[4,4],[3,1],[1,2]], [], [[2,7]]),
];

const ex3IntermediateCheckpoints = [
  makeWorld(15, 10, 9, 1, 'right', ex3Trees, [], []),
  makeWorld(15, 10, 9, 7, 'left',  ex3Trees, [], []),
];

// Each example is a self-contained Challenge object (matches the same
// shape as user-authored challenges in store.js). The four examples are
// bundled into one book; menu entries are shortcuts into this book.
function makeExampleChallenge({
  id, name, world, fsm, blocks, python, notesByMode, targetWorld,
  intermediateCheckpoints = [], limit, endOnTargetNotRequired = false,
}) {
  return {
    id,
    guid: normaliseGuid(id, 'challenge'),
    name,
    mode: 'blocks',
    ...
    limits: {
      enforced: true,
      blocks: { added: limit.blocks },
      fsm:    { states: limit.fsmStates, transitions: limit.fsmTransitions },
      python: { tokens: limit.python },
    },
    noCheckTarget: false,
...
const EXAMPLES = [
  makeExampleChallenge({
    ...
    intermediateCheckpoints: ex1IntermediateCheckpoints,
    limit: exampleLimits.ex1,
  }),
  makeExampleChallenge({
    id: 'builtin:ex2',
  ...
  makeExampleChallenge({
    ...
    intermediateCheckpoints: ex3IntermediateCheckpoints,
    limit: exampleLimits.ex3,
  }),
  makeExampleChallenge({
    id: 'builtin:ex4',
    ...
    limit: exampleLimits.ex4,
  }),
];

const firstChallenge = EXAMPLES[0];
const book = {
  karaWebVersion: 5,
  appMode: 'blocks',
  name: 'KaraWeb Examples',
  savedAt: new Date('2026-06-14T00:00:00Z').toISOString(),
  world: firstChallenge.initialWorld,
  blocks: { blocklyState: null },
  challengeFileGuid: normaliseGuid('karaweb-examples-book', 'book'),
  challenges: EXAMPLES,
  challengeWork: {},
};
...
const outFile = join(outDir, 'examples.json');
const json = JSON.stringify(book, null, 2);
writeFileSync(outFile, `${json}\n`);

const kb = (json.length / 1024).toFixed(1);
console.log(`wrote ${EXAMPLES.length} examples (${kb} KB) to ${outFile}`);
